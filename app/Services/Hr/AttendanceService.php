...
class AttendanceService
{
    ...
    public function upsertRoster(Carbon $date, array $rows, ?string $defaultShiftId = null): int
    {
        ...
        // A day generated by an APPROVED leave request is not the roster's to
        // overwrite. Marking such a day "present" left the leave still deducted
        // from the balance while payroll counted it as worked — the leave and
        // attendance records then disagreed with no trace of why. Cancel or
        // withdraw the leave request instead; that removes the day properly.
        ...
        if ($onLeave !== []) {
            $indexes = collect($rows)
                ->filter(fn (array $row) => in_array($row['employee_id'] ?? null, $onLeave, true))
                // A row that leaves the day as leave is harmless; only a change
                // away from it is refused.
                ->filter(fn (array $row) => ($row['status'] ?? null) !== AttendanceStatus::OnLeave->value)
                ->keys();

            if ($indexes->isNotEmpty()) {
                throw ValidationException::withMessages(
                    $indexes->mapWithKeys(fn ($i) => [
                        "rows.{$i}.status" => __('hr.validation.attendance_on_approved_leave'),
                    ])->all()
                );
            }
        }

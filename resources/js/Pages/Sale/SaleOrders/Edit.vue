...
const saleOrderData = computed(() => props.saleOrder?.data ?? props.saleOrder ?? {})
...
const buildInitialRows = () => (saleOrderData.value.item_list || []).map((item) => ({
...
const form = useForm({
    number: saleOrderData.value.number,
    customer_id: saleOrderData.value.customer_id,
    selected_ledger: saleOrderData.value.customer ? { id: saleOrderData.value.customer_id, name: saleOrderData.value.customer_name } : '',
    date: '',
    delivery_date: '',
    currency_id: saleOrderData.value.currency_id || '',
    selected_currency: findById(currencies.value?.data, saleOrderData.value.currency_id) || '',
    rate: saleOrderData.value.rate || '',
    warehouse_id: saleOrderData.value.warehouse_id || '',
    selected_warehouse: findById(warehouses.value?.data, saleOrderData.value.warehouse_id) || '',
    discount: saleOrderData.value.discount || '',
    discount_type: saleOrderData.value.discount_type || 'percentage',
    note: saleOrderData.value.note || '',
    item_list: [],
...
const handleSubmit = () => {
    ...
    form.put(route('sale-orders.update', saleOrderData.value.id))
}
        ...
        <form @submit.prevent="handleSubmit">
            <div class="mb-5 rounded-xl border border-violet-500 p-4 shadow-sm relative">
                <div class="absolute -top-3 ltr:left-3 rtl:right-3 bg-card px-2 text-sm font-semibold text-muted-foreground text-violet-500">
                    {{ t('general.edit', { name: t('sale_order.sale_order') }) }} #{{ saleOrderData.number }}
                </div>

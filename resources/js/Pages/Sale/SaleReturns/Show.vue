...
<template>
    <AppLayout :title="`${t('sale_return.sale_return')} #${returnData.number}`">
        <div class="space-y-6">
            <ShowPageToolbar
                back-route="sale-returns.index"
                :status="returnData.status"
                :edit-route="returnData.id ? route('sale-returns.edit', returnData.id) : null"
                edit-permission="sale_returns.update"
            ...
            <TransactionActionDialog
                ...
                :description="t('sale_return.post_sale_return_description')"
            ...
            <TransactionActionDialog
                ...
                :description="t('sale_return.reverse_sale_return_description')"
                @confirm="reverseSaleReturn"
            />
            ...
            <fieldset class="rounded-xl border border-border bg-card px-5 pb-5 pt-3 text-card-foreground shadow-sm">
                <legend class="px-2 flex items-center gap-1.5">
                    <span class="text-sm font-semibold text-violet-500">{{ t('sale_return.sale_return') }} #{{ returnData.number }}</span>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="text-sm font-medium text-foreground">{{ returnData.customer_name || '-' }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <FileCheck class="h-3 w-3" />{{ t('sale_return.linked_sale') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">
                            <a v-if="returnData.sale_id" :href="route('sales.show', returnData.sale_id)" class="text-violet-600 underline dark:text-violet-400">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <FileCheck class="h-3 w-3" />{{ t('sale_return.reason') }}
                        </div>

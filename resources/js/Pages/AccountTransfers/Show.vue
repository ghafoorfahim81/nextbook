...
function postTransfer() {
    submit(route('account-transfers.post', transferData.value.id), {}, {
        preserveScroll: true,
...
function reverseTransfer(reason) {
    submit(route('account-transfers.reverse', transferData.value.id), { reason }, {
        preserveScroll: true,
...
<template>
    <AppLayout :title="`${t('general.account_transfer')} #${transferData.number}`">
        ...
        <div class="mt-4">
            <AttachmentList :items="transferData.attachments || []" :label="t('general.attachments')" />
        </div>
    </AppLayout>
</template>

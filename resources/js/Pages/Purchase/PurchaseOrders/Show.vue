...
const orderData = computed(() => props.purchaseOrder?.data ?? props.purchaseOrder ?? {});
...
const postPurchaseOrder = () => {
    router.post(route('purchase-orders.post', orderData.value.id), {}, {
        preserveScroll: true,
        onSuccess: () => {
...
const cancelPurchaseOrder = () => {
    router.post(route('purchase-orders.cancel', orderData.value.id), {}, {
        preserveScroll: true,
        onSuccess: () => { cancelDialogOpen.value = false },
    });
};

const canCancel = computed(() => orderData.value.status === 'draft' && can('purchase_orders.update'));
</script>

<template>
    <AppLayout :title="`${t('purchase_order.purchase_order')} #${orderData.number}`">
        <div class="space-y-6">
            <ShowPageToolbar
                back-route="purchase-orders.index"
                :status="orderData.status"
                :edit-route="orderData.id ? route('purchase-orders.edit', orderData.id) : null"
                edit-permission="purchase_orders.update"
                @post="postDialogOpen = true"
            >
                <Button v-if="canCancel" variant="destructive" size="sm" @click="cancelDialogOpen = true">
                    {{ t('purchase_order.cancel_purchase_order') }}
                </Button>
            </ShowPageToolbar>
            ...
            <TransactionActionDialog
                ...
                :description="t('purchase_order.post_purchase_order_description')"
                @confirm="postPurchaseOrder"
            />
            <ConfirmDeleteDialog
                v-model:open="cancelDialogOpen"
                :title="t('purchase_order.cancel_purchase_order')"
                :description="t('purchase_order.cancel_purchase_order_description')"
                :cancel-text="t('general.cancel')"
                :continue-text="t('purchase_order.cancel_purchase_order')"
            ...
            <fieldset class="rounded-xl border border-border bg-card px-5 pb-5 pt-3 text-card-foreground shadow-sm">
                <legend class="px-2 flex items-center gap-1.5">
                    <span class="text-sm font-semibold text-violet-500">{{ t('purchase_order.purchase_order') }} #{{ orderData.number }}</span>
                    <Badge :class="statusBadgeClasses">{{ orderData.status_label }}</Badge>
                    <a v-if="orderData.purchase_id" :href="route('purchases.show', orderData.purchase_id)">
                        <Badge class="border-violet-500/30 bg-violet-500/10 text-violet-700 dark:text-violet-300 hover:underline">
                            {{ t('purchase.purchase') }} #{{ orderData.purchase_number }}
                        </Badge>
                    </a>
                </legend>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <Truck class="h-3 w-3" />{{ t('purchase_order.delivery_date') }}
                        </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="text-sm font-medium text-foreground">{{ orderData.supplier_name || '-' }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <FileCheck class="h-3 w-3" />{{ t('purchase_order.linked_purchase') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">
                            <a v-if="orderData.purchase_id" :href="route('purchases.show', orderData.purchase_id)" class="text-violet-600 underline dark:text-violet-400">
                                #{{ orderData.purchase_number }}
                            </a>
                            <span v-else>-</span>
                        </div>
                    </div>
                ...
                <div v-if="orderData.status === 'posted' && can('purchases.create')" class="flex justify-end mt-4">
                    <Button variant="outline" size="sm" @click="router.visit(route('purchases.create', { purchase_order_id: orderData.id }))">
                        {{ t('purchase_order.convert_to_purchase') }}
                    </Button>
                </div>
            </fieldset>

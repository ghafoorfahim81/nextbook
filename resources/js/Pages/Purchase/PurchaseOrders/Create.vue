...
const props = defineProps({
    ...
    purchaseOrderNumber: { type: [String, Number], required: true },
})
...
// Single reactive copy of the purchase_order preferences so the panel and form stay in sync live.
const purchasePrefs = reactive(JSON.parse(JSON.stringify(user_preferences.value?.purchase_order ?? {})))
...
const form = useForm({
    number: props.purchaseOrderNumber,
    supplier_id: '',
    selected_ledger: '',
    date: '',
    delivery_date: '',
...
const applyCreateDefaults = ({ number = props.purchaseOrderNumber } = {}) => {
...
const resetFormForCreate = ({ number = props.purchaseOrderNumber } = {}) => {
    form.reset()
    form.clearErrors()
    form.item_list = []
...
watch(() => props.purchaseOrderNumber, (newNumber) => {
...
function handleSubmit({ createAndNew = false } = {}) {
    if (!form.items[0]?.selected_item) {
        notifySound('error');
        toast({
            title: t('purchase.no_item_warning') || t('sale.no_item_warning'),
            description: t('purchase.no_item_warning_description') || t('sale.no_item_warning_description'),
            variant: 'destructive',
    ...
    form.transform((data) => ({ ...data, ...payload })).post(route('purchase-orders.store'), {
        onSuccess: () => {
            notifySound('success');
            if (createAndNew) {
                resetFormForCreate();
            ...
            toast({
                ...
                description: t('general.create_success', { name: t('purchase_order.purchase_order') }),
                variant: 'success',
        ...
        onError: (errors) => {
            ...
            toast({
                ...
                description: firstServerError(errors) ?? t('general.error_creating_purchase_order'),
                variant: 'destructive',
...
<template>
    <AppLayout :title="t('general.create', { name: t('purchase_order.purchase_order') })" :sidebar-collapsed="true">
        <FormPageToolbar
        ...
        />
        <FormPreferencesPanel module="purchase_order"
            v-model:open="showPreferencesPanel"
            pref-group="purchase_order"
            :prefs="purchasePrefs"
            :title="t('purchase_order.purchase_order')"
        />
        ...
        <form ref="saveFormRef" @submit.prevent="handleSubmitAction('create')">
            <div class="mb-5 rounded-xl border border-violet-500 p-4 shadow-sm relative">
                <div class="absolute -top-3 ltr:left-3 rtl:right-3 bg-card px-2 text-sm font-semibold text-muted-foreground text-violet-500">{{ t('general.create', { name: t('purchase_order.purchase_order') }) }}</div>
            ...
            <div class="mt-5 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-2 items-start">
                ...
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    <div class="text-sm font-semibold mb-3 text-violet-500">{{ t('purchase_order.order_total') }}</div>

...
const returnData = computed(() => props.purchaseReturn?.data ?? props.purchaseReturn ?? {});

const totalAmount = computed(() =>
    Number(returnData.value.items?.reduce((sum, item) => sum + Number(item.line_total || 0), 0) || 0)
);
const totalQuantity = computed(() =>
    Number(returnData.value.items?.reduce((sum, item) => sum + Number(item.quantity || 0), 0) || 0)
);
const formattedTotalAmount = computed(() => totalAmount.value.toFixed(2));
const formatLineValue = (value) => Number(value || 0).toFixed(2);
const currencySymbol = computed(() => returnData.value.transaction?.currency?.symbol || '');

const statusBadgeClasses = computed(() => {
    switch (returnData.value.status) {
...
const postPurchaseReturn = () => {
    router.post(route('purchase-returns.post', returnData.value.id), {}, {
        preserveScroll: true,
        onSuccess: () => {
...
const reversePurchaseReturn = (reason) => {
    router.post(route('purchase-returns.reverse', returnData.value.id), { reason }, {
        preserveScroll: true,
...
<template>
    <AppLayout :title="`${t('purchase_return.purchase_return')} #${returnData.number}`">
        <div class="space-y-6">
            <ShowPageToolbar
                back-route="purchase-returns.index"
                :status="returnData.status"
                :edit-route="returnData.id ? route('purchase-returns.edit', returnData.id) : null"
                edit-permission="purchase_returns.update"
            ...
            <TransactionActionDialog
                ...
                type="post"
                :title="t('purchase_return.post_purchase_return')"
                :description="t('purchase_return.post_purchase_return_description')"
                @confirm="postPurchaseReturn"
            />
            <TransactionActionDialog
                v-model:open="reverseDialogOpen"
                type="reverse"
                :title="t('purchase_return.reverse_purchase_return')"
                :description="t('purchase_return.reverse_purchase_return_description')"
                @confirm="reversePurchaseReturn"
            />
            ...
            <fieldset class="rounded-xl border border-border bg-card px-5 pb-5 pt-3 text-card-foreground shadow-sm">
                <legend class="px-2 flex items-center gap-1.5">
                    <span class="text-sm font-semibold text-violet-500">{{ t('purchase_return.purchase_return') }} #{{ returnData.number }}</span>
                    <Badge :class="statusBadgeClasses">{{ getStatusLabel(returnData.status) }}</Badge>
                </legend>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    <div class="space-y-1.5">
                        ...
                        <div class="text-sm font-medium text-foreground">{{ returnData.date }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <User class="h-3 w-3" />{{ t('ledger.supplier.supplier') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">{{ returnData.supplier_name || '-' }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <FileCheck class="h-3 w-3" />{{ t('purchase_return.linked_purchase') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">
                            <a v-if="returnData.purchase_id" :href="route('purchases.show', returnData.purchase_id)" class="text-violet-600 underline dark:text-violet-400">
                                #{{ returnData.purchase_number }}
                            </a>
                            <span v-else>-</span>
                        </div>
                    </div>
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <FileCheck class="h-3 w-3" />{{ t('purchase_return.reason') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">{{ returnData.reason_label || '-' }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <User class="h-3 w-3" />{{ t('general.created_by') }}
                        </div>
                        <div class="text-sm font-medium text-foreground">{{ returnData.created_by?.name || '-' }}</div>
                    </div>
                </div>
                <div v-if="returnData.description" class="mt-4 text-sm text-muted-foreground">

...
const form = useForm({
    ...
    purchase_id: returnData.value.purchase_id,
    date: '',
...
const existingQuantityByPurchaseItemId = computed(() => {
    const map = {}
    for (const item of returnData.value.item_list || []) {
        map[item.purchase_item_id] = Number(item.quantity) || 0
    }
    return map
})
...
const loadReturnableItems = async () => {
    ...
    try {
        const { data } = await axios.get(route('purchase-returns.returnable-items'), {
            params: {
                purchase_id: returnData.value.purchase_id,
                exclude_purchase_return_id: returnData.value.id,
            },
        })

        rows.value = (data.items || []).map(item => ({
            ...item,
            quantity_to_return: existingQuantityByPurchaseItemId.value[item.purchase_item_id] ?? '',
        }))
    } catch (error) {
        rows.value = []
...
const handleSubmit = () => {
    ...
    form.put(route('purchase-returns.update', returnData.value.id))
}
...
<template>
    <AppLayout :title="t('general.edit', { name: t('purchase_return.purchase_return') })">
        ...
        <form ref="saveFormRef" @submit.prevent="handleSubmit">
            <div class="mb-5 rounded-xl border border-violet-500 p-4 shadow-sm relative">
                <div class="absolute -top-3 ltr:left-3 rtl:right-3 bg-card px-2 text-sm font-semibold text-muted-foreground text-violet-500">
                    {{ t('general.edit', { name: t('purchase_return.purchase_return') }) }} #{{ returnData.number }}
                </div>
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                    <NextInput disabled :model-value="`#${returnData.purchase_number}`" :label="t('purchase_return.linked_purchase')" />
                ...
                <table v-else class="w-full table-fixed min-w-[1100px] border-separate">
                    <thead class="bg-card sticky top-0 z-10">
                        <tr class="rounded-xl text-muted-foreground font-semibold text-sm text-violet-500">
                            ...
                            <th class="px-2 py-2 w-28">{{ t('item.variant') }}</th>
                    ...
                    <tbody>
                        <tr v-for="(row, index) in rows" :key="row.purchase_item_id"
                            ...
                            <td class="px-2 py-2">{{ row.variant_name || '-' }}</td>

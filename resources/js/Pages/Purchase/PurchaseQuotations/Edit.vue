...
const props = defineProps({
    purchaseQuotation: { type: Object, required: true },
})

const purchaseQuotationData = computed(() => props.purchaseQuotation?.data ?? props.purchaseQuotation ?? {})
...
const purchasePrefs = reactive(JSON.parse(JSON.stringify(user_preferences.value?.purchase_quotation ?? {})))
...
const buildInitialRows = () => (purchaseQuotationData.value.item_list || []).map((item) => ({
...
const form = useForm({
    number: purchaseQuotationData.value.number,
    supplier_id: purchaseQuotationData.value.supplier_id,
    selected_ledger: purchaseQuotationData.value.supplier ? { id: purchaseQuotationData.value.supplier_id, name: purchaseQuotationData.value.supplier_name } : '',
    date: '',
    valid_until: '',
    currency_id: purchaseQuotationData.value.currency_id || '',
    selected_currency: findById(currencies.value?.data, purchaseQuotationData.value.currency_id) || '',
    rate: purchaseQuotationData.value.rate || '',
    warehouse_id: purchaseQuotationData.value.warehouse_id || '',
    selected_warehouse: findById(warehouses.value?.data, purchaseQuotationData.value.warehouse_id) || '',
    discount: purchaseQuotationData.value.discount || '',
    discount_type: purchaseQuotationData.value.discount_type || 'percentage',
    note: purchaseQuotationData.value.note || '',
    item_list: [],
...
const loadItemOptions = async (warehouseId = form.warehouse_id) => {
    try {
        const response = await axios.get(route('search.items-list'), {
            params: { warehouse_id: warehouseId || undefined, limit: 50, in_stock_only: false }
        })
...
const handleSubmit = () => {
    ...
    form.put(route('purchase-quotations.update', purchaseQuotationData.value.id))
}
...
<template>
    <AppLayout :title="t('general.edit', { name: t('purchase_quotation.purchase_quotation') })">
        <FormPageToolbar
            back-route="purchase-quotations.index"
            module="purchase_quotations"
            :show-preferences="true"
            @preferences="showPreferencesPanel = true"
        />
        <FormPreferencesPanel module="purchase_quotation"
            v-model:open="showPreferencesPanel"
            pref-group="purchase_quotation"
            :prefs="purchasePrefs"
        ...
        <form @submit.prevent="handleSubmit">
            <div class="mb-5 rounded-xl border border-violet-500 p-4 shadow-sm relative">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
                    ...
                    <NextDate v-model="form.valid_until" :error="form.errors?.valid_until" :placeholder="t('general.enter', { text: t('purchase_quotation.valid_until') })" :label="t('purchase_quotation.valid_until')" />
            ...
            <div class="mt-5 grid grid-cols-1 md:grid-cols-2 gap-2 items-start">
                ...
                <div class="rounded-xl border bg-card p-4 shadow-sm">
                    <div class="text-sm font-semibold mb-3 text-violet-500">{{ t('purchase_quotation.quotation_total') }}</div>
            ...
            <SubmitButtons
                ...
                module="purchase_quotations"
                @cancel="$inertia.visit(route('purchase-quotations.index'))"
            />
        </form>
    </AppLayout>
</template>

...
const props = defineProps({
    ...
    purchaseQuotationNumber: { type: [String, Number], required: true },
})
...
// Single reactive copy of the purchase_quotation preferences so the panel and form stay in sync live.
...
const form = useForm({
    number: props.purchaseQuotationNumber,
    supplier_id: '',
    selected_ledger: '',
    date: '',
    valid_until: '',
...
const applyCreateDefaults = ({ number = props.purchaseQuotationNumber } = {}) => {
...
const resetFormForCreate = ({ number = props.purchaseQuotationNumber } = {}) => {
    form.reset()
    form.clearErrors()
    form.item_list = []
...
watch(() => props.purchaseQuotationNumber, (newNumber) => {
...
function handleSubmit({ createAndNew = false } = {}) {
    ...
    form.transform((data) => ({ ...data, ...payload })).post(route('purchase-quotations.store'), {
        onSuccess: () => {
            notifySound('success');
            if (createAndNew) {
                resetFormForCreate();
            ...
            toast({
                ...
                description: t('general.create_success', { name: t('purchase_quotation.purchase_quotation') }),
                variant: 'success',
        ...
        onError: () => {
            ...
            toast({
                ...
                description: t('general.error_creating_purchase_quotation'),
                variant: 'destructive',
...
<template>
    <AppLayout :title="t('general.create', { name: t('purchase_quotation.purchase_quotation') })" :sidebar-collapsed="true">
        <FormPageToolbar
        ...
        <form @submit.prevent="handleSubmitAction('create')">
            <div class="mb-5 rounded-xl border border-violet-500 p-4 shadow-sm relative">
                <div class="absolute -top-3 ltr:left-3 rtl:right-3 bg-card px-2 text-sm font-semibold text-muted-foreground text-violet-500">{{ t('general.create', { name: t('purchase_quotation.purchase_quotation') }) }}</div>
            ...
            <SubmitButtons module="purchase_quotations"
                :create-label="t('general.create')"
                :create-and-new-label="t('general.create_and_new')"
                :cancel-label="t('general.cancel')"
                :creating-label="t('general.creating', { name: t('purchase_quotation.purchase_quotation') })"

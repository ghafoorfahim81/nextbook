...
const quotationData = computed(() => props.purchaseQuotation?.data ?? props.purchaseQuotation ?? {});
...
const canCancel = computed(() => quotationData.value.status !== 'cancelled' && can('purchase_quotations.update'));
const printUrl = computed(() => quotationData.value.id ? route('purchase-quotations.print', quotationData.value.id) : null);
</script>

<template>
    <AppLayout :title="`${t('purchase_quotation.purchase_quotation')} #${quotationData.number}`">
        <div class="space-y-6">
            <ShowPageToolbar
                back-route="purchase-quotations.index"
                :status="quotationData.status"
                :edit-route="quotationData.id ? route('purchase-quotations.edit', quotationData.id) : null"
                edit-permission="purchase_quotations.update"
                :print-url="printUrl"
                @post="postDialogOpen = true"
            >
                <Button v-if="canCancel" variant="destructive" size="sm" @click="cancelDialogOpen = true">
                    {{ t('purchase_quotation.cancel_purchase_quotation') }}
                </Button>
            </ShowPageToolbar>
            ...
            <TransactionActionDialog
                ...
                type="post"
                :title="t('purchase_quotation.post_purchase_quotation')"
                :description="t('purchase_quotation.post_purchase_quotation_description')"
                @confirm="postPurchaseQuotation"
            />
            <ConfirmDeleteDialog
                v-model:open="cancelDialogOpen"
                :title="t('purchase_quotation.cancel_purchase_quotation')"
                :description="t('purchase_quotation.cancel_purchase_quotation_description')"
                :cancel-text="t('general.cancel')"
                :continue-text="t('purchase_quotation.cancel_purchase_quotation')"
                @confirm="cancelPurchaseQuotation"
            />
            ...
            <fieldset class="rounded-xl border border-border bg-card px-5 pb-5 pt-3 text-card-foreground shadow-sm">
                <legend class="px-2 flex items-center gap-1.5">
                    <span class="text-sm font-semibold text-violet-500">{{ t('purchase_quotation.purchase_quotation') }} #{{ quotationData.number }}</span>
                ...
                <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4">
                    ...
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">
                            <CalendarClock class="h-3 w-3" />{{ t('purchase_quotation.valid_until') }}
                        </div>
                    ...
                    <div class="space-y-1.5">
                        ...
                        <div class="text-sm font-medium text-foreground">{{ quotationData.supplier_name || '-' }}</div>
                    </div>
                    <div class="space-y-1.5">
                        <div class="flex items-center gap-2 text-xs text-muted-foreground">

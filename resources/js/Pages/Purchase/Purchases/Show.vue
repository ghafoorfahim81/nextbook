...
import { Package2, FileText, User, Calendar, DollarSign, FileCheck, ArrowLeft, SquarePen, Download, Send, RotateCcw } from 'lucide-vue-next';
...
const postPurchase = () => {
    ...
    postForm.post(route('purchases.post', purchaseData.value.id), {
...
const reversePurchase = () => {
    reverseForm.reason = reverseReason.value;
    reverseForm.post(route('purchases.reverse', purchaseData.value.id), {
        ...
        <div class="space-y-6">
            ...
            <div class="flex flex-wrap items-center justify-between gap-3">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <Button
                        v-if="can('purchases.update') && purchaseData.status === 'draft'"
                        size="sm"
                    ...
                    <Button
                        v-if="can('purchases.update') && purchaseData.status === 'posted'"
                        variant="destructive"
                        size="sm"
                        class="gap-1.5"
            ...
            <div v-if="showReverseDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
                <div class="w-full max-w-md rounded-xl border border-border bg-card p-6 shadow-lg">
                    ...
                    <div class="flex justify-end gap-2">
                        ...
                        <Button variant="destructive" size="sm" :disabled="reverseForm.processing" @click="reversePurchase">
            ...
            <fieldset class="rounded-xl border border-border bg-card px-5 pb-5 pt-3 text-card-foreground shadow-sm">
                <legend class="px-2 flex items-center gap-1.5">
                    ...
                    <Badge :class="statusBadgeClasses">{{ purchaseData.status }}</Badge>
                </legend>

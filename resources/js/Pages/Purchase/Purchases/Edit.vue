...
const paymentLine = (purchaseRecord?.transaction?.lines || []).find(
    (line) => bankAccountIds.has(line.account_id) && toNum(line.credit, 0) > 0
);

const initialRows = (purchaseRecord?.items || []).map((item) => {
    ...
    const baseUnit = Number(selectedItem?.unitMeasure?.unit) || 1;
    const selectedUnit = Number(selectedMeasure?.unit) || baseUnit;
    const rate = toNum(purchaseRecord?.rate, 1) || 1;
...
const form = useForm({
    number: purchaseRecord.number || '',
    supplier_id: purchaseRecord.supplier_id || '',
    date: purchaseRecord.date || '',
    currency_id: purchaseRecord.currency_id || '',
    rate: purchaseRecord.rate || '',
    purchase_type: purchaseRecord.sale_purchase_type_id || '',
    selected_currency: findById(props.currencies?.data, purchaseRecord.currency_id) || purchaseRecord.transaction?.currency || '',
    selected_ledger: purchaseRecord.supplier || '',
    selected_purchase_type: findById(props.salePurchaseTypes, purchaseRecord.sale_purchase_type_id) || '',
    selected_bank_account: findById(props.bankAccounts, paymentLine?.account_id || purchaseRecord.bank_account_id) || '',
    bank_account_id: paymentLine?.account_id || purchaseRecord.bank_account_id || '',
    discount: purchaseRecord.discount ?? '',
    transaction_total: purchaseRecord.transaction_total || 0,
    discount_total: 0,
    discount_type: purchaseRecord.discount_type || 'percentage',
    description: purchaseRecord.description || '',
    payment: paymentLine ? {
        method: 'cash',
        amount: toNum(paymentLine.credit, 0),
...
const itemSearchOptions = computed(() => {
    ...
    additionalParams.in_stock_only = false;
...
const handleSelectChange = (field, value) => {
    ...
    if (field === 'purchase_type' && value?.id === 'cash') {
        handleResetPayment();
    }
...
const handleItemChange = (index, selectedItem) => {
    ...
    row.on_hand = resolveVariantOnHand(row) ?? selectedItem.on_hand;
    ...
    row.unit_price = toDocumentCurrency((toNum(row.base_unit_price, 0) * selectedUnit) / baseUnit, form.rate);
    row.selected_item = selectedItem;
...
watch(() => form.rate, (newRate) => {
    ...
    form.items.forEach((row) => {
        ...
        const baseUnit = Number(row.selected_item?.unitMeasure?.unit) || 1;
        const selectedUnit = Number(row.selected_measure?.unit) || baseUnit;
        const baseUnitPrice = toNum(row.base_unit_price ?? row.selected_item?.purchase_price ?? row.selected_item?.avg_cost, 0);
...
const rowTotal = (index) => {
    const item = form.items[index];
    if (!item || !item.selected_item) return '';
    return Number((toNum(item.quantity, 0) * toNum(item.unit_price, 0) - toNum(item.item_discount, 0) + toNum(item.tax, 0)).toFixed(2));
};
...
const billDiscountCurrency = computed(() => {
    ...
    if (form.discount_type === 'percentage') {
        return goodsTotal.value * (billDiscount / 100);
    }
    return billDiscount;
});
...
const billDiscountPercent = computed(() => {
    ...
    const totalGoods = goodsTotal.value;
    return totalGoods > 0 ? (billDiscount / totalGoods) * 100 : 0;
});
    ...
    <AppLayout :title="t('general.edit', { name: t('purchase.purchase') })" :sidebar-collapsed="true">
        ...
        <form @submit.prevent="handleSubmitAction">
                ...
                <table class="w-full table-fixed min-w-[1200px] purchase-table border-separate">
                    ...
                    <tbody class="p-2">
                        <tr v-for="(item, index) in form.items" :key="`${index}-${item.item_id || 'row'}`" class="hover:bg-muted/40 transition-colors">
                            ...
                            <td :class="{ 'opacity-50 pointer-events-none select-none': !isRowEnabled(index) }" v-if="item_columns.expiry">
                                <NextDate
            ...
            <SubmitButtons module="purchase"
                ...
                :disabled="hasDuplicateRows || stockLocked"

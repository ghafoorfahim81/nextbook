...
<template>
  <AppLayout :title="t('general.edit', { name: t('receipt.receipt') })">
    ...
    />
    <form @submit.prevent="confirmSave('receipt', () => submit('update'))">
      <div class="mb-5 rounded-xl border p-4 shadow-sm border-primary relative">
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mt-3">
          ...
          />
          <NextInput v-if="rpFields.currency" placeholder="Rate" :error="form.errors?.rate" :disabled="form.selected_currency?.is_base_currency === true" type="number" step="any" v-model="form.rate" :label="t('general.rate')" />
          <NextSelect
      ...
      <div class="mt-4 flex gap-2">
        ...
        <button type="button" class="btn btn-primary px-4 py-2 rounded-md bg-primary text-white" :disabled="form.processing" @click="confirmSave('receipt', () => submit('save_and_print'))">
